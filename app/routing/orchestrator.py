...
class Orchestrator:
    ...
    async def route_and_execute(self, query: str, feedback_intent: str = None, headers: dict = None, background_tasks = None) -> dict:
        ...
        if not is_informative(query):
            logger.info("Low information query refused", query=query)
        ...
        if self.adapter_mgr.has_adapter(domain):
            logger.info("Using adapter for domain", domain=domain)
        ...
        # Path B: Knowledge Gate (General/Unknown)
            ...
            logger.info("KnowledgeGate Decision", decision=decision.upper(), semantic=semantic_conf, has_evidence=has_evidence)
    ...
    async def _execute_external_rag(self, query: str, collector, response_data: dict):
        ...
        try:
            ...
            logger.warning("RAG timed out", query=query)
