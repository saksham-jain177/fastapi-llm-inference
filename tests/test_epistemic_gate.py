...
os.environ["USE_MOCKED_MODELS"] = "true"
...
def test_estimate_confidence_high_agreement():
    ...
    # Mock generate_fn to return identical responses
    mock_gen = MagicMock(return_value="The capital of France is Paris.")
    
    response, confidence = estimator.estimate_confidence("What is the capital of France?", mock_gen)
...
def test_estimate_confidence_low_agreement():
    ...
    # Mock generate_fn to return different responses
    responses = [
        "Response A",
        "Response B",
        "Response C",
    ...
    # Since low_temp_agreement might be low, it might exit early or continue to phase 2
    # If phase 1 < 0.75, it returns early.

def test_estimate_confidence_perturbation_refusal():
    estimator = ConfidenceEstimator()
    
    # Mock responses: Phase 1 is consistent, Phase 2 (perturbed) refuses
...
def test_orchestrator_decision_flow():
    ...
    # Mock dependencies
    with patch("app.models.quantized.load_model") as mock_load, \
         patch("app.models.calibration.get_confidence_threshold", return_value=0.7), \
         patch("app.routing.retrieval_gate.get_retrieval_gate") as mock_gate:
        
        # Mock model and tokenizer
        mock_model = MagicMock()
        mock_tokenizer = MagicMock()
        mock_tokenizer.decode.return_value = "Assistant: Paris"
        mock_load.return_value = (mock_model, mock_tokenizer)
        
        # Mock responses from model.generate
        ...
        assert result["mode"] in ["internal-confident", "abstained", "rag"]
